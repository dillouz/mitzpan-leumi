...
| `EMAIL_WEBHOOK_URL`    | URL לשליחת אימייל אישור חתימה (transactional)                         | לא     | ריק            |
...
### 2. אימייל אישור חתימה (transactional)

אותה שיטה — הגדירו `EMAIL_WEBHOOK_URL` שיקבל payload:
```json
{
  "email":      "david@example.com",
  "first_name": "דוד",
  "last_name":  "כהן",
  "subject":    "תודה שחתמת על המצפן הלאומי",
  "text":       "שלום דוד,\n\nתודה שהצטרפת...",
  "html":       "<div dir=\"rtl\">...</div>"
}
```
האימייל נשלח **לכל מי שמסר כתובת אימייל**, ללא קשר להסכמה לדיוור —
זהו אישור קבלה לפעולה שהמשתמש ביצע (transactional), לא דיוור שיווקי.
...
- **ולידציה**: שם פרטי + שם משפחה + (טלפון או אימייל). `consent` הוא אופציונלי ומסומן מראש — לא חוסם חתימה.
...
3. עברו ל-Environment ומלאו את המשתנים המסומנים `sync: false`:
   ...
   - `CRM_WEBHOOK_URL`, `EMAIL_WEBHOOK_URL`
