 ...
 *   RESEND_API_KEY    – מפתח Resend API לשליחת אימייל אישור ישירות (מומלץ)
 *   RESEND_FROM       – כתובת שולח מאומתת ב-Resend (למשל: "המצפן הלאומי <no-reply@yourdomain.com>")
 *   EMAIL_WEBHOOK_URL – fallback: URL webhook לשליחת אימייל (Make/Zapier/SendGrid)
...
const RESEND_FROM        = process.env.RESEND_FROM || 'המצפן הלאומי <onboarding@resend.dev>';
...
/**
 * שליחה ישירה דרך Resend API (https://resend.com) — ללא תלויות חיצוניות.
 * Resend מציע 3,000 אימיילים בחינם בחודש ו-100/יום בחינם.
 * להפעלה:
 *   1. צרו חשבון ב-resend.com
 *   2. הוסיפו דומיין מאומת (או השתמשו ב-onboarding@resend.dev לבדיקות)
 *   3. צרו API Key והגדירו RESEND_API_KEY ב-Render
 *   4. הגדירו RESEND_FROM לכתובת השולח המאומתת
 */
function sendViaResend(to, subject, text, html) {
  const payload = JSON.stringify({ from: RESEND_FROM, to, subject, text, html });
  const req = require('https').request({
    method:   'POST',
    ...
    resp.on('end', () => {
      if (resp.statusCode >= 400) {
        console.error('[resend-error]', resp.statusCode, raw.slice(0, 500));
      }
    });
  });
...
/**
 * שליחת אימייל אישור חתימה (transactional).
 * נשלח לכל מי שמסר כתובת אימייל — זהו אישור קבלה לפעולה שהמשתמש ביצע,
 * ולכן אינו תלוי בהסכמה לדיוור שיווקי (consent).
 *
 * סדר עדיפויות לשליחה:
 *   1. RESEND_API_KEY — שליחה ישירה דרך Resend API (מומלץ)
 *   2. EMAIL_WEBHOOK_URL — fallback ל-Make/Zapier/SendGrid webhook
 *   3. אם אף אחד לא מוגדר — לא נשלח כלום (לוג בלבד)
 */
function sendEmailConfirmation(email, firstName, lastName) {
  if (!email) return;
  const safeFirst = String(firstName || '').slice(0, 100);
  const safeLast  = String(lastName  || '').slice(0, 100);
  const subject = 'תודה שחתמת על המצפן הלאומי';
  const text =
    `שלום ${safeFirst},\n\n` +
    'תודה שהצטרפת אלינו למאבק על הדרך הלאומית-ליברלית. ' +
    'החתימה שלך נקלטה בהצלחה במערכת.\n\n' +
    'הדרך הזו היא העתיד של ישראל.\n\n' +
    'בברכה,\n' +
    'דן אילוז';
  const html =
    `<div dir="rtl" style="font-family:Arial,Heebo,sans-serif;line-height:1.7;color:#11223F;max-width:560px;margin:0 auto;padding:24px">` +
    `<h2 style="color:#0B1E3F;margin:0 0 16px">תודה שחתמת, ${escapeHtml(safeFirst)}!</h2>` +
    `<p>החתימה שלך על המצפן הלאומי נקלטה בהצלחה במערכת.</p>` +
    `<p>תודה שהצטרפת אלינו למאבק על הדרך הלאומית-ליברלית. הדרך הזו היא העתיד של ישראל.</p>` +
    `<p style="margin-top:28px"><strong>דן אילוז</strong></p>` +
    `</div>`;

  if (RESEND_API_KEY) {
    sendViaResend(email, subject, text, html);
  } else if (EMAIL_WEBHOOK_URL) {
    postWebhook(EMAIL_WEBHOOK_URL, { email, first_name: safeFirst, last_name: safeLast, subject, text, html });
  } else {
    console.log('[email] no RESEND_API_KEY or EMAIL_WEBHOOK_URL configured — skipping confirmation to', email);
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === '/api/sign' && method === 'POST') {
    ...
    // Consent is tracked but NOT required — we still store `consent` on the entry
    // so the email newsletter can filter by it (opted-in recipients only).
    ...
    // אימייל אישור חתימה — transactional, נשלח לכל מי שמסר אימייל
    sendEmailConfirmation(emailLow, first_name, last_name);
...
server.listen(PORT, () => {
  ...
  if (RESEND_API_KEY)    console.log(`  📧 Resend:        מחובר (from: ${RESEND_FROM})`);
  else if (EMAIL_WEBHOOK_URL) console.log('  📧 Email webhook: מחובר');
  else                   console.log('  ⚠️  אימייל אישור: לא מוגדר (הגדירו RESEND_API_KEY או EMAIL_WEBHOOK_URL)');
